...
export function QuizCard({ quiz, index }: { quiz: QuizCardType; index: number }) {
  return (
    <Link href={`/quiz/${quiz.slug}`} className="w-full h-full block">
      <Card className="group pt-0 gap-0 pb-2 relative w-full h-full overflow-hidden border-0 bg-white dark:bg-gray-900 rounded-3xl shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-300">
        {/* Top gradient banner */}
